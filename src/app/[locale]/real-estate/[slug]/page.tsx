...
import { getRealEstateItemBySlug } from "@/features/real-estate/api";
...
export async function generateMetadata({ params }: Props) {
  ...
  try {
    const item = await getRealEstateItemBySlug(slug, normalizedLocale);

    return buildMetadata({
      title: item.title,
      description:
        item.excerpt ||
        (normalizedLocale === "en" ? `${item.title} real estate detail` : `${item.title} 부동산 상세`),
      path: `/${normalizedLocale}/real-estate/${slug}`,
    });
  } catch {
    return buildMetadata({
      title: normalizedLocale === "en" ? "Real Estate Detail" : "부동산 상세",
      description:
        normalizedLocale === "en" ? "Real estate detail page" : "부동산 상세 페이지",
      path: `/${normalizedLocale}/real-estate/${slug}`,
    });
  }
}
...
export default async function RealEstateDetailPage({ params }: Props) {
  ...
  try {
    item = await getRealEstateItemBySlug(slug, normalizedLocale);
  } catch {
    notFound();
  }
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-3 text-sm text-neutral-700 md:grid-cols-2">
              {item.listingType ? (
                <div>
                  <strong className="mr-2 text-neutral-900">
                    {normalizedLocale === "en" ? "Listing Type" : "매물 유형"}
                  </strong>
              ...
              {item.propertyType ? (
                <div>
                  <strong className="mr-2 text-neutral-900">
                    {normalizedLocale === "en" ? "Property Type" : "부동산 유형"}
                  </strong>
              ...
              {item.bedrooms ? (
                <div>
                  <strong className="mr-2 text-neutral-900">
                    {normalizedLocale === "en" ? "Bedrooms" : "침실"}
                  </strong>
              ...
              {item.bathrooms ? (
                <div>
                  <strong className="mr-2 text-neutral-900">
                    {normalizedLocale === "en" ? "Bathrooms" : "욕실"}
                  </strong>
