...
import { getBusinessSaleItemBySlug } from "@/features/business-sale/api";
...
export async function generateMetadata({ params }: Props) {
  ...
  try {
    const item = await getBusinessSaleItemBySlug(slug, normalizedLocale);
    ...
    return buildMetadata({
      ...
      description:
        item.excerpt ||
        (normalizedLocale === "en"
          ? `${item.title} business sale detail`
          : `${item.title} 사업체매매 상세`),
      path: `/${normalizedLocale}/business-sale/${slug}`,
    });
  } catch {
    return buildMetadata({
      title: normalizedLocale === "en" ? "Business Sale Detail" : "사업체매매 상세",
      description:
        normalizedLocale === "en"
          ? "Business sale detail page"
          : "사업체매매 상세 페이지",
      path: `/${normalizedLocale}/business-sale/${slug}`,
    });
  }
...
export default async function BusinessSaleDetailPage({ params }: Props) {
  const { locale, slug } = await params;
  const normalizedLocale = locale === "en" ? "en" : "ko";
  ...
  try {
    item = await getBusinessSaleItemBySlug(slug, normalizedLocale);
  } catch {
    notFound();
  }
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-3 text-sm text-neutral-700 md:grid-cols-2">
              ...
              {item.businessCategory ? (
                <div>
                  <strong className="mr-2 text-neutral-900">
                    {normalizedLocale === "en" ? "Category" : "업종"}
                  </strong>
              ...
              {item.monthlyRevenue ? (
                <div>
                  <strong className="mr-2 text-neutral-900">
                    {normalizedLocale === "en" ? "Revenue" : "월매출"}
                  </strong>
